    const app = {
      ...
      bindEvents() {
        ...
        this.$sessionForm.addEventListener('submit', (event) => {
          ...
          // Vérifier que l'heure est un multiple de 15 minutes
          const minutes = datetime.getMinutes();
          if (minutes % 15 !== 0) {
            this.toast('L\'heure doit être un multiple de 15 minutes (ex: 9h00, 9h15, 9h30, 9h45)', true);
            return;
          }
      ...
      openSessionModal(session = null) {
        ...
        if (session) {
          ...
          this.$sessionSubmit.textContent = 'Créer';
        ...
        this.state.sessions.forEach((session) => {
          ...
          header.innerHTML = `
            ...
            <div class="header-pills">
              ...
              <span class="session-price-pill"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg> ${priceLabel}</span>
            </div>
          `;
          ...
          peopleGrid.innerHTML = `
            <div class="people-col">
              <div class="people-header"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> <span class="${isFull ? 'people-count-full' : ''}">${participantCount}/${session.capacity}</span> Participants</div>
              ${participantsList}
            </div>
            <div class="people-col">
              <div class="people-header"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg> Intéressés</div>
              ${followersList}
            </div>
          `;
      ...
      checkInstallPrompt() {
        ...
        if (isStandalone) {
          // L'app est déjà installée, pas besoin d'afficher le prompt
      ...
      dismissInstallPrompt(permanently = false) {
        ...
        // "Plus tard" : Pas de cookie, le popup réapparaîtra à la prochaine connexion
      },
      getCookie(name) {
      ...
      async clearAppBadge() {
        if ('clearAppBadge' in navigator) {
          try {
            ...
            console.log('Badge API non supporté');
          }
        }
      },
      async setupPushNotifications() {
        ...
        try {
          ...
          if (permission === 'granted') {
            ...
            // Demander la permission après un court délai pour ne pas être intrusif
      ...
      async requestNotificationPermission() {
        try {
          ...
          if (permission === 'granted') {
            this.toast('Notifications activées ! Vous serez notifié des nouvelles sessions.');
            await this.subscribeToPush();
          } else if (permission === 'denied') {
      ...
      addToCalendarAutomatically(session) {
        ...
        if (this.isIOSDevice()) {
          // iOS: télécharger un fichier .ics pour Apple Calendar
      ...
      buildCalendarEventDetails(session) {
        ...
        const description = [
          `Niveau : ${session.level || 'Non précisé'}`,
          `Organisateur : ${session.organizer}`,
          `Tarif : ${priceLabel}`,
          `Durée : ${this.formatDuration(session.durationMinutes)}`,
